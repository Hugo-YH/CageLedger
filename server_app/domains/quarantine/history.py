        ...
        for test in tests:
            ...
            for supplier in {s["supplier"] for s in sources.values()}:
                ...
                matching = [
                    ...
                    if s["supplier"] == supplier and (not filters.get("species") or filters["species"] in s["species"])
                ]
                matching = [
                    s
                    for s in matching
                    if (
                        not filters.get("dateFrom")
                        or (test["testDate"] if filters.get("dateType") == "test" else s["intakeDate"])
                        >= filters["dateFrom"]
                    )
                    and (
                        not filters.get("dateTo")
                        or (test["testDate"] if filters.get("dateType") == "test" else s["intakeDate"])
                        <= filters["dateTo"]
                    )
                ]
