...
def get_release_announcement_acknowledgements(handler, _params):
    user = handler.current_user()
    if not user:
        return JsonResponse({"error": "请先登录"}, HTTPStatus.UNAUTHORIZED)
    handler._actor_id = user["id"]
    with connect_db() as conn:
        return JsonResponse(release_announcement_acknowledgements(conn, user["id"]))


def post_release_announcement_acknowledgements(handler, _params):
    user = handler.current_user()
    if not user:
        return JsonResponse({"error": "请先登录"}, HTTPStatus.UNAUTHORIZED)
    handler._actor_id = user["id"]
    try:
        ...
        with connect_db() as conn:
            return JsonResponse(acknowledge_release_announcements(conn, user["id"], versions))
...
API_ROUTER.add("GET", r"/api/release-announcements", get_release_announcement_acknowledgements)
API_ROUTER.add("POST", r"/api/release-announcements/acknowledge", post_release_announcement_acknowledgements)
API_ROUTER.add(
    "GET",
