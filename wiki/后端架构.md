...
- SQLite 启用 WAL，运行数据默认位于 `data/database/cageledger.sqlite`。
...
服务端生成 PDF，缓存目录默认位于 `data/cache/pdf/`。巡检和报销附件保存在 `data/files/` 下的受鉴权目录，通过 API 下载。生产镜像预装 Chromium 与中文字体，PDF 任务复用常驻浏览器进程。
