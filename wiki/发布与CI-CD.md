...
| 工作流                                   | 作用                                                  |
| ---------------------------------------- | ----------------------------------------------------- |
| `.gitea/workflows/ci.yml`                | PR 和 main 的轻量格式、lint、类型、单元测试和构建门禁 |
...
`ci.yml` 使用单个轻量 job：Prettier、ESLint、Stylelint、Markdownlint、ShellCheck、Ruff、TypeScript、Vitest、生产构建和架构门禁。

Mac mini 执行 Python 全量测试、完整 Playwright、API 冒烟、PDF/打印验收和多架构镜像构建。Gitea Runner 保持低内存运行，不安装 Chromium 或 Playwright 浏览器镜像。
