...
IACUC 汇总表导入仅更新可同步的业务记录：当前结算版本已发起、已交回、已提交财务、已归档或已锁定时，其关联数量统计表不被导入覆盖（包括项目字段、负责人及更新时间）。优先按版本中的 `sourceIds/sourceId` 确定关联；旧版本缺少来源 ID 时按月份、负责人及已有伦理范围兼容匹配。未发起或已撤回到已生成状态的记录沿用原同步规则。
