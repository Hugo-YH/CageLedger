...
- 每个操作区最多一个 `primary`；常规操作使用默认按钮；明确标记的低频操作进入“更多”，行内通常最多两个常用动作。单据跟踪保留固定三槽：左侧登记使用实心蓝色主按钮、查看使用淡蓝色按钮，中间撤回使用 danger，右侧锁定使用实心紫色白字，解锁使用淡紫色按钮，与已锁定状态标签保持语义一致；不可用的槽位留空，不把唯一附加动作收进“更多”。破坏性操作使用 danger 并保留确认。
