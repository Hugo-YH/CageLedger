...
```bash
CAGELEDGER_IMAGE_TAG=0.4.9g docker compose up -d
```
...
「数据管理 -> 系统更新」默认关闭最新发布版检查，不会从网页自动拉取并运行新代码。
...
```text
dist/CageLedger-offline-v0.4.9g.tar.gz
```
...
```bash
cd /volume1/docker
tar -xzf CageLedger-offline-v0.4.9g.tar.gz
cd CageLedger
...
docker pull git.cellnucle.us/hugo/cageledger:0.4.9g
```
