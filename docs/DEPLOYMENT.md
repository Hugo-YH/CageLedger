...
```bash
CAGELEDGER_IMAGE_TAG=0.4.6c docker compose up -d
```
...
```text
dist/CageLedger-offline-v0.4.6c.tar.gz
```
...
```bash
cd /volume1/docker
tar -xzf CageLedger-offline-v0.4.6c.tar.gz
cd CageLedger
...
docker pull ghcr.io/hugo-yh/cageledger:0.4.6c
```
