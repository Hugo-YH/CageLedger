...
```bash
CAGELEDGER_IMAGE_TAG=0.4.9f docker compose up -d
```
...
```text
dist/CageLedger-offline-v0.4.9f.tar.gz
```
...
```bash
cd /volume1/docker
tar -xzf CageLedger-offline-v0.4.9f.tar.gz
cd CageLedger
...
docker pull git.cellnucle.us/hugo/cageledger:0.4.9f
```
