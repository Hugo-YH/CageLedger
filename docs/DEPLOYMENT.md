...
```bash
CAGELEDGER_IMAGE_TAG=0.4.0 docker compose up -d
```
...
```text
dist/CageLedger-offline-v0.4.0.tar.gz
```
...
```bash
cd /volume1/docker
tar -xzf CageLedger-offline-v0.4.0.tar.gz
cd CageLedger
...
docker pull ghcr.io/hugo-yh/cageledger:0.4.0
```
