...
```bash
CAGELEDGER_IMAGE_TAG=0.3.9a docker compose up -d
```
...
```text
dist/CageLedger-offline-v0.3.9a.tar.gz
```
...
```bash
cd /volume1/docker
tar -xzf CageLedger-offline-v0.3.9a.tar.gz
cd CageLedger
...
docker pull ghcr.io/hugo-yh/cageledger:0.3.9a
```
