...
| 打印和 PDF                           | `pdf`                            | 渲染检查 A4、尺寸、页数和二维码                                                         |
| CSV、XLSX 和模板                     | `spreadsheets`                   | 保留表头、公式、格式和导入兼容                                                          |
...
- `npm run check` 是提交和发布前的快速门禁。
- `npm run verify:full` 执行快速门禁、生产构建和完整 Playwright。
