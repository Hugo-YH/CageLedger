...
export const DOCUMENT_RELEASE_NOTES: ReleaseNote[] = [
  ...
  },
  {
    ...
    title: "全系统界面与交互更新",
    items: [
      "按 Ant Design 统一工作页面、登录与公开查询页面的字体、控件尺寸、颜色、间距和操作层级",
      "整理深色侧栏、页面工具栏与行内操作，常用操作直接显示，低频操作集中到“更多”",
      "改善数量统计表、检测记录和巡检等长表单的吸顶、窄屏布局与错误反馈，切换主题时保留草稿和焦点",
      "列表首次加载显示骨架，后台刷新保留当前内容和筛选操作；继续保留跨页选择与范围变化清空规则",
      "完善浅色、深色、跟随系统和减少动画模式，统一登录、扫码详情和文档站的基础视觉",
      "“系统更新”按版本从旧到新汇总上次确认之后的更新，避免后续小修复覆盖错过的大版本说明",
      "多个版本在一个弹窗内查看，版本较多时支持展开收起，大版本和最新版本默认展开",
      "确认状态按账号保存，换浏览器或设备后保持一致；确认失败保留弹窗并支持重试",
      "没有历史确认记录的账号仅展示当前版本，完整历史仍可随时查阅",
      "增加 Firefox、WebKit 桌面兼容专项，覆盖全部工作入口及代表性筛选、保存、上传、主题与权限场景",
      "保留 Chromium 全量业务回归、手机视口及扫码模拟视频验证；WebKit 自动化结果不等同真实 Safari 实测",
      "沿用现有业务规则、权限、检疫报告内容和笼卡纸质打印尺寸",
    ],
  },
  {
    version: "1.3.0",
    ...
    title: "检疫管理与报告",
    items: [
      "新增检疫批次、寄生虫、ELISA、PCR 检测管理，以及报告台账和供应商历史查询",
      "已接收动物可归入检疫批次，登记实际采样与检测结果，完成后统一更新覆盖动物的检疫状态",
      "检测记录支持筛选查询、原始资料上传与按供应商复检，登记、编辑和详情页面统一使用 Ant Design 工作区",
      "三类检疫报告按历史原表模板导出 PDF，支持草稿核对、正式出具、更正版本和历史下载",
      "独立保留报告内容快照、原始资料和操作记录，便于追溯；备份时需同时保存 SQLite 与 `data/files/`",
      "扫码支持同一画面识别多个二维码，冻结画面后选择目标，并可返回重新选择或继续扫码",
      "整理编辑与批量操作区域，改善列表刷新反馈、窄屏操作布局和未保存内容提示",
      "待检疫动物的操作说明收进标题旁的帮助气泡，减少页面提示占用",
    ],
  },
  {
    version: "1.2.14",
  ...
  },
  {
    ...
    title: "后端安全、并发与稳定性优化",
    items: [
      "修复报销明细可跨单据引用、核销并发确认可能超额以及 `NaN` 等非法金额进入计算的问题，核销额度改为数据库原子校验与写入",
      "加固登录、会话和写接口：增加分账户与分来源登录失败限流、过期会话清理、可配置安全 Cookie、严格同源校验及受控跨域响应",
      "统一请求体、JSON 和文件上传解析，拒绝截断正文、非法长度、非标准数值、畸形 multipart、重复文件字段与传输编码歧义",
      "修复 SQLite 连接及测试进程句柄未及时释放的问题；缓存键改为确定性摘要，支持空结果缓存并使用单调时钟计算有效期",
      "API 增加请求标识、安全响应头、统一系统错误边界和脱敏结构化访问日志，慢请求可按路由、状态、耗时和安全用户标识定位",
      "完成 253 项前端测试、250 项后端测试、生产构建、性能基准和浏览器回归；本地开发代理同步适配严格来源校验",
    ],
  },
  {
    version: "1.2.7",
  ...
  },
  {
    ...
    title: "列表列宽与操作区修复",
    items: [
      "恢复业务列表的列宽调整能力；选择列也可调整，修改单列时不再带动其他业务列，列宽偏好继续按列表保存",
      "待接收批次、已保存数量统计表、结算管理和单据跟踪的操作列固定在可视区域右侧；横向滚动统一由表格内部处理，避免操作按钮被裁切或移出工作区",
      "表头统一左对齐，选择列居中；表格内容按文本、数值和操作语义对齐，操作按钮统一主次层级、尺寸和排列顺序",
      "浏览器回归增加列宽独立性、滚动容器归属、固定列位置和按钮完整显示检查",
    ],
  },
  {
    version: "1.2.1",
  ...
  },
  {
    ...
    build: "189",
    releasedAt: "2026-08-30",
    title: "临时饲养间笼卡与报销登记控件优化",
    items: [
      "珠江新城设施 8014 临时饲养间使用专用笼卡版式，A4 每页排 15 张，不显示二维码、房间和笼位；饲养周期按入驻日期和饲养周期天数计算",
      "普通饲养间继续使用每页 14 张的原版式；两种笼卡不能混合打印，选中混合批次时打印按钮会停用并说明原因",
      "两种版式在不足整页时都会询问是否补空白卡，原有 IACUC 匹配、品系标准化和每笼 5 只的分卡规则保持不变",
      "交回登记和报销单补录中的删除操作改为标准按钮，与同行输入框统一为 32px 高度；零金额流程不再显示报销单相关内容，并补充键盘操作和四档视口回归",
    ],
  },
  {
    version: "1.1.1",
  ...
  },
  {
    ...
    title: "PDF 生成链路与系统状态体验优化",
    items: [
      "PDF 缓存改为同键并发只生成一次，并使用唯一临时文件和原子替换；用户导出优先于后台预热，缓存按容量和保留时长自动清理",
      "Chromium 渲染增加字体就绪等待、优先级队列、超时处理和耗时统计；结算 PDF 去除重复缓存与重复写入",
      "“关于系统”新增 PDF 队列、生成耗时、失败与超时、缓存命中率和容量监控；页面按内容区宽度响应式排版，修复标题字号未生效及中等视口逐字断行的问题",
      "结算单预览为伦理号增加明确标签和自然换行；1180px 以下将伦理号与操作按钮上下分区，避免文字遮挡、截断",
      "PDF 回归测试新增真实 A4 物理页数、空白尾页、页码和前后端模板一致性校验",
    ],
  },
  {
    version: "1.0.20",
    ...
    title: "系统性能监控与查询优化",
    items: [
      "后端新增当前服务进程的请求、缓存和 SQLite 性能统计，支持慢请求与慢数据库操作阈值；API 冒烟和性能基准同步覆盖运行指标",
      "“关于系统”重建为管理员运行状态页，集中展示缓存命中率、HTTP 延迟、SQLite 延迟与锁状态；普通账号不请求运维数据，指标改为手动刷新，避免监控请求干扰统计",
      "优化结算候选、接收批次筛选、工作台概览和状态读取链路，补充结构化索引、快照缓存与失效规则，降低重复查询和大列表开销",
      "工作台图表进入视口后再加载，减少首页初次渲染负担；系统状态页与巡检模块补齐响应式、权限和无障碍回归",
      "修复巡检评估模块帮助按钮受全局最小高度影响而显示为椭圆的问题，桌面和平板统一为 24px 圆形按钮",
    ],
  },
  {
    version: "1.0.19",
  ...
  },
  {
    ...
    build: "177",
    releasedAt: "2026-08-19",
    title: "汇总导出与单据跟踪报销信息联动",
    items: [
      "结算管理入口“月度饲养费汇总”更名为“汇总导出”，页面标题与提示同步更新",
      "导出工作簿新增“报销金额（元）”“报销备注”列：报销金额为单据跟踪交回登记或补录报销单的金额汇总，经费本编号、单号、金额、备注仅取单据跟踪已登记且未撤回的报销单，未登记时为空，多个报销单号全部填入单号列",
      "导出工作簿在科研报销单经费本号前新增“结算单”列，显示登记的交回状态（已交回或空）",
      "导出查询排除已撤回流程的残留登记，作废报销单不会进入汇总",
    ],
  },
  {
    version: "1.0.11",
  ...
  },
  {
    ...
    title: "单据跟踪与交回登记优化",
    items: [
      "待接收批次编辑弹窗滚动修复：弹窗高度受视口约束，内容在弹窗内稳定滚动，表头保持固定",
      "单据跟踪列表筛选修复：已筛选一列后其他列仍可继续筛选，修复后端标量列筛选选项的 SQL 拼接错误",
      "交回登记与补录弹窗新增“经费本编号”字段：下拉展示支撑经费完整描述，选择后填写并保存纯经费本编号，可自由输入新编号；金额默认填入该单据结算金额",
      "已归档撤回后再次登记自动回填上次登记内容（开关、报销单明细、备注），并修复不同流程间表单数据串用",
      "登记弹窗经费本编号与报销单号列宽统一，金额输入框移除上下箭头",
      "查看归档流程记录中“结算单/报销单交回”改为卡片化展示：结算单卡片（状态、备注）与报销单卡片（状态、报销单子卡片、总金额、备注）",
      "报销单交回状态增加分支：已交回但未全额缴费时标记为“报销单 已交回（未全额缴费）”，列表与归档详情保持一致",
    ],
  },
  {
    version: "1.0.9",
  ...
  },
  {
    ...
    title: "结算单据与数量录入体验修复",
    items: [
      "饲养费结算汇总表说明区统一为同行的加粗标题与正文；多品系收费标准横向展示，自定义收费与伦理到期提示保持清晰分行",
      "汇总表按品系展示笼数或只数，支持小鼠减免与梯度收费；结算金额、表头高度、表格边线和 PDF 缓存同步修复",
      "单据跟踪支持已发起后的锁定、解锁、补录与归档；撤回、交回登记和历史时间线保留完整备注与状态记录",
      "数量统计表弹窗修复滚动归属和字段控件高度，基础信息与 IACUC 信息布局统一；笼位编辑字段统一命名为“笼位编号”",
      "工作台与业务页面骨架屏对齐真实布局，补充 Ant Design 样式自检与打印、PDF、业务流程回归测试",
    ],
  },
  {
    version: "1.0.7",
    ...
    title: "结算汇总表与加载体验优化",
    items: [
      "饲养费结算汇总表按动物品系分组展示伦理明细；按只计费品系统一显示“只数”，小鼠保留笼数、减免与梯度计费明细",
      "汇总表说明新增当前表内品系的收费标准；小鼠梯度价格仅在实际发生梯度收费时展示，伦理到期提示沿用原有到期与临期规则",
      "收费标准与支撑经费展示优化：同一经费编号自动去重，单一品系省略序号，结算单标题移除减免笼数附注",
      "工作台、结算候选列表和仪表盘骨架屏贴合实际页面布局；仪表盘图表按需加载，导航预取保持轻量",
      "修复结算汇总表、PDF、Excel 与打印预览的品系字段一致性，并补充相应模板与导出回归测试",
    ],
  },
  {
    version: "1.0.6",
    ...
    title: "结算流程状态机与单据跟踪",
    items: [
      "结算与报销台账重构为“单据跟踪”：以结算流程为主线（已生成 → 已发起 → 已归档，每步可退回上一状态），移除原结算应收/报销单/核销中心/历史台账四页签",
      "阶段边界拆分：结算管理展示未发起/已生成/已发起，单据跟踪只展示已发起及之后的流程",
      "交回登记重做：结算单/报销单开关展开对应区域，报销单按“报销单号 + 金额”成对登记，实收金额自动按报销单汇总；结算金额为 0 的流程不显示报销单开关",
      "已归档流程支持“补录报销单”，实收金额按全部报销单重新汇总，流程保持已归档",
      "查看归档改为“流程记录”时间轴：数量统计表登记、汇总表生成、发起、交回等环节，历史重复记录默认折叠",
      "单据跟踪列表支持排序、筛选、分页，新增 IACUC、登记人员列，筛选支持多选",
      "术语统一：撤回（退回上一状态，白底红字）与撤销（删除流程，红底白字）区分按钮样式并加图标",
      "结算通知弹窗恢复一次性“复制并确认”，发起后预览弹窗按钮立即置灰",
      "菜单与工作区改名：录入数量统计表、结算管理、单据跟踪；隐藏未启用的动态笼位图入口",
      "列表筛选多选、时间显示格式、Ant Design 滑块样式修复",
      "文档同步更新：结算与报销、业务流程、工作台导航、用户操作手册、API 契约等",
    ],
  },
  {
    version: "1.0.5",
  ...
  },
  {
    ...
    title: "1.x 正式发布",
    items: [
      "全系统工作区完成 Ant Design 适配：桌面、平板和移动端共享一致的导航、表单、表格、弹窗、反馈和无障碍交互；主题、语义色、布局与样式归属由统一 Token 和质量门禁维护",
      "笼位、笼卡和接收流程支持批量打印、批量确认接收、待进驻任务衔接与跨页操作，IACUC 带入、到期提示、负责人历史提示和服务端品系标准化覆盖录入全链路",
      "动物巡检覆盖标准目录草稿编辑、版本发布、参考图、现场附件、异常处置、复查、权限控制与 PDF 报告，目录与附件保留完整审计和历史兼容能力",
      "数量统计表、项目负责人结算和报销台账支持服务端筛选、登记人员、结算状态、撤回、通知邮件模板、批量 PDF/Excel 导出、打印预览、月度汇总与核销跟踪",
      "总览页按真实接收、数量统计表和结算快照展示运营数据；关键列表、IACUC 检索、Dashboard 聚合、静态资源与批量写入完成性能优化",
      "运行数据按数据库、索引、附件、缓存、备份、导入、归档和失败文件分层管理；Docker 与群晖部署继续使用宿主机 `./data` 到容器 `/app/data` 的持久映射",
      "发布链路固定执行质量检查、生产构建、浏览器回归、离线包、双架构容器镜像、Gitea Release 和 Wiki 同步；主分支成为正式版本唯一出口",
    ],
    note: "本条汇总 `1.0.0（Build 127）`、`1.0.0-beta1` 至 `1.0.0-beta21` 及 `1.0.0-rc1` 至 `1.0.0-rc5` 的全部改动。以下版本明细保留为开发与公测记录；Build 127 是历史内部构建记录，正式标签从 `v1.0.1` 开始。",
  },
  {
    version: "1.0.0-rc5",
    build: "155",
  ...
  },
  {
    ...
    title: "总览页运营数据卡片与品系标准化优化",
    items: [
      "总览页改为真实业务数据卡片：接收动物统计（批次、只数、品系分布）、饲养间笼位统计（逐日笼位与月度笼日）、饲养费统计（按项目负责人结算口径），移除基于笼位图的概览卡片",
      "月份选择器支持「历史合计」与具体月份倒序切换：具体月份下接收动物统计按日展示、饲养间按日趋势、饲养费按当月口径；历史合计下展示全量累计，饲养间按月度趋势",
      "品系分布新增标准化：C57/B6J、C57BL/6JGpt、3w 等变体统一为 C57BL/6J，转基因小鼠、基因鼠、c57基因小鼠统一为基因工程小鼠，历史数据在聚合时自动归一",
      "品系分布改为「Top 5 + 其他」玫瑰图，数量取对数映射花瓣半径、Ant Design 图表色板与渐变填充，小值品类清晰可见",
      "饲养间笼位统计改为平滑渐变面积图，tooltip 显示各房间自身笼位数而非堆叠累计值",
      "饲养费统计改为 Ant Design Progress 渐变进度条排名列表，两列布局展示前十课题组的金额与伦理号数",
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      "修复不参与减免的动物（如大鼠）在按负责人合表中仍被分配免费笼的问题，免费额度只落到符合减免规则的伦理号",
      "优化优先减免语义：仅开启开关不填笼数时，该伦理号在剩余额度分配中优先；优先伦理号不再重复获得二次分配额度；同伦理多张表的优先笼数按伦理号合并校验",
      "结算单说明只提示已到期或 3 个月内即将到期的伦理号，时态以生成日期为参照区分已到期（已于）与未到期（将于）",
      "修复结算单汇总表金额列与总笼数列数字溢出单元格的问题，打印行高压缩避免整月明细把页脚挤到下一页，汇总列表头简化为「笼数｜减免｜梯度｜缴纳（元）」",
      "PDF 导出缓存 key 纳入渲染代码指纹，样式或排版代码改动后旧缓存自动失效",
      "文档站 LLMs.md 按 llms.txt 规范对齐并输出每页 Markdown，修复「编辑此页」链接分支",
    ],
  },
  {
    ...
    title: "笼位管理界面按 Ant Design 设计规范重新设计",
    items: [
      "笼位状态配色改为 Ant Design 功能色：空为中性灰、已预约为信息蓝、在用为成功绿、未填结束日期为警示琥珀、超期饲养为错误红，图例/笼位卡/状态点/待进驻标签统一",
      "笼位固定为 120px 正方形默认大小，窄笼架不再被拉长，宽笼架自动横向滚动",
      "移除「全部/在用/已预约/空」状态筛选按钮；「多选录入」移入顶部工具栏并置为主色，批量操作按钮随模式显示",
      "编辑笼位/批量编辑/待进驻弹窗按内容定宽（640/720），修复表单宽度溢出弹窗",
      "巡检标准编辑器移除已废弃的评分字段，保留「异常类型」选项编辑（与录入表单「正常/异常」判定一致）",
      "「按项目负责人结算」标题移至「结算数据来源」上方，标题风格与核销工作台统一",
    ],
  },
  {
    version: "1.0.0-rc2",
  ...
  },
  {
    ...
    title: "巡检标准在线编辑器",
    items: [
      "「巡检标准」页新增管理员在线编辑：模块 → 分类 → 子分类 → 条目四级目录树，节点表单支持名称、描述、排序、输入类型、评分标准、子选项、处置建议与参考图维护，保存为草稿并校验后发布生效",
      "目录版本历史与回滚：可查看全部历史版本，并将指定历史版本的内容发布为新版本立即生效，旧版本保留",
      "参考图管理：上传即写入运行数据目录并绑定节点引用，支持替换、移除与缩略图预览",
      "目录数据清理与展示配置化：移除被隐藏或重复的巡检条目，异常动物（小鼠）评估的区域归属、分组与改名全部由目录数据驱动，录入表单与标准页计数一致",
      "权限控制：编辑、发布、回滚均为管理员专属，房间管理员保持只读",
    ],
  },
  {
    version: "1.0.0-rc1",
