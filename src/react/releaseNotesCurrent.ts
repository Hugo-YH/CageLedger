...
export const CURRENT_RELEASE_NOTES: ReleaseNote[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "结算单说明只提示已到期或 3 个月内即将到期的伦理号，不再列出远期到期；时态区分已到期（已于）与未到期（将于），以生成日期为参照",
      "修复结算单汇总表金额列与总笼数列数字溢出单元格的问题，金额列与首列分配更宽列宽；打印行高压缩，整月明细不再把页脚挤到下一页",
      "结算单汇总列表头简化为「笼数｜减免｜梯度｜缴纳（元）」",
      "PDF 导出缓存 key 纳入渲染代码指纹，样式或排版代码改动后旧缓存自动失效，无需手动升级版本号",
    ],
  },
  {
    version: "0.8.4",
    releasedAt: "2026-08-05 17:30",
    title: "结算单 Excel 导出与结算列表筛选修复",
    items: [
      "项目负责人结算列表新增「导出 Excel」：版式与打印/PDF 一致（标题、信息区、汇总块、按 IACUC 分列、单项合计、本月待缴纳总计、自定义收费明细、签字区），金额与合计写公式可联动重算；批量导出按负责人一人一个文件打包 ZIP，文件名与 PDF 命名规则一致",
      "修复含自定义收费的结算单 Excel 应付总计重复计入自定义金额；未实际产生梯度收费的列不再显示「梯度收费」标记，与 PDF 一致",
      "修复结算列表月份、负责人等列筛选导致的服务端报错（快照表与数量统计表联查列名歧义），组合筛选恢复正常",
      "待接收批次列表状态筛选项汉化为 未打印/已打印/已接收",
    ],
  },
  {
    version: "0.8.3",
  ...
  },
  {
    ...
    items: [
      "数量统计表录入的 IACUC 匹配改为前缀优先并限制候选条数，空输入不再加载全部伦理索引，伦理输入与 31 行录入区相互隔离避免连带重渲染",
      "保存统计表只校验当前表和受影响的镜像表，无转出转入时跳过申请数据全量读取，大月份保存不再对整月所有表逐日重算",
      "确认保存按钮在请求期间显示“正在保存…”，保存完成后清空录入区并给出结果提示，避免无反馈的假死感",
      "结算候选列表支持表头全选当前筛选结果与工具栏全选/清空按钮，跨页批量导出和发起结算共用同一套分页读取",
    ],
  },
  {
    version: "0.7.8",
