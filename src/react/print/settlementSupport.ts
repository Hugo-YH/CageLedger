...
const settlementSpeciesOrder: Record<string, number> = {
  小鼠: 0,
  大鼠: 1,
  豚鼠: 2,
  兔: 3,
...
export function compareSettlementSpecies(left: string, right: string) {
  return (
    (settlementSpeciesOrder[left] ?? Number.MAX_SAFE_INTEGER) -
    (settlementSpeciesOrder[right] ?? Number.MAX_SAFE_INTEGER)
  );
}

export function settlementSpeciesHeaders(
  slots: Array<{ column: { speciesLabel: string } | null }>,
  showMouseSummary: boolean,
  groupGridUnits: number,
) {
  const groups: Array<{ speciesLabel: string; span: number }> = [];
  slots.forEach((slot) => {
    const speciesLabel = slot.column?.speciesLabel || "";
    const current = groups.at(-1);
    if (current?.speciesLabel === speciesLabel) current.span += groupGridUnits;
    else groups.push({ speciesLabel, span: groupGridUnits });
  });
  if (showMouseSummary && groups[0]?.speciesLabel === "小鼠") groups[0].span += groupGridUnits;
  return groups
    .map((group) =>
      group.speciesLabel
        ? `<th colspan="${group.span}">${escapeHtml(group.speciesLabel)}</th>`
        : `<th colspan="${group.span}" class="group-empty-cell"></th>`,
    )
    .join("");
}
