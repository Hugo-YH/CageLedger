...
export function intakeCardsPrintHtml(batches: IntakeBatch[]) {
  ...
  const pages = Array.from({ length: Math.ceil(items.length / INTAKE_CARD_PRINT_PAGE_SIZE) }, (_, index) => items.slice(index * INTAKE_CARD_PRINT_PAGE_SIZE, (index + 1) * INTAKE_CARD_PRINT_PAGE_SIZE));
  return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><title>接收笼卡打印</title><style>${printStyles()}</style></head><body>${pages.map((page) => `<div class="sheet">${page.map(({ batch, card }) => renderCard(batch, card)).join("")}</div>`).join("")}</body></html>`;
}

export function openIntakeCardPrint(batches: IntakeBatch[]) {
...
function renderCard(batch: IntakeBatch, card: IntakeBatch["cards"][number]) {
  const qrId = String(card.qrId || "").trim().toUpperCase();
  return `<section class="card"><table><colgroup><col style="width:16mm"><col style="width:11mm"><col style="width:23mm"><col style="width:18mm"><col style="width:8mm"><col style="width:7mm"><col style="width:8.5mm"><col style="width:8.5mm"></colgroup>
...
<tr style="height:3.91mm"><td class="row-head">日期</td><td class="row-head">数目变化</td><td class="row-head">饲养周期</td><td class="row-head" colspan="3">房间</td><td class="qr-cell" colspan="2" rowspan="4">${qrCodeSvg(qrId, "笼卡二维码")}</td></tr>
<tr style="height:5.26mm"><td class="value">${escapeHtml(formatDate(batch.intakeDate))}</td><td class="value">${escapeHtml(card.suggestedQuantity)}</td><td class="cycle">${escapeHtml(formatRange(batch.intakeDate, batch.endDate))}</td><td class="room" colspan="3" rowspan="3">${escapeHtml(batch.roomName)}</td></tr>
