...
export function SupplierHistoryView({ onOpen }: { onOpen: (batchId: string) => void }) {
  ...
  return (
    <>
      <Space wrap>
        <Input.Search aria-label="查询供应商" placeholder="供应商" onSearch={(v) => set("supplier", v)} allowClear />
        <Select
          aria-label="历史日期类型"
        ...
        />
        <Input.Search aria-label="历史动物种类" placeholder="动物种类" onSearch={(v) => set("species", v)} allowClear />
        <Select
          aria-label="历史检测方法"
          placeholder="全部方法"
          allowClear
