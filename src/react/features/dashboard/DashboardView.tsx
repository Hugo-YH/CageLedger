      ...
      <section className="workspace-head">
        <div className="workspace-head-main">
          ...
          <div className="workspace-title-line">
            <h1>实验动物笼位管理与计费系统</h1>
            <span className="workspace-status-badge">当前版本 v{APP_VERSION}</span>
          </div>
          <p className="workspace-summary">
            围绕接收、入驻、结算和流程推进组织日常工作，首屏直接显示本周待办、异常项和两设施口径。
          </p>
          <div className="workspace-meta-strip">
            ...
            <Meta
              ...
              tone={value("exceptionCount") ? "warning" : "success"}
            />
          </div>
        </div>
      </section>
      <div className="workspace-body dashboard-workspace-body">
        ...
        <div className="dashboard-overview-grid">
          ...
          <Overview
            ...
            note="预留或正式入驻后会从当前房间待办中移除。"
            onClick={() => navigate("cages")}
          />
          <Overview
            title="本月待办流程"
            value={value("currentMonthWorkflowTodoCount")}
            note="已生成、已发送、已签字交回都会留在待办链。"
            onClick={() => navigate("workflow-center")}
          />
          <Overview
            title="异常项"
            value={value("exceptionCount")}
            note={`房间未匹配 ${value("unmatchedIntakeCount")} · 待进驻超期 ${value("overduePlacementCount")} · 流程待推进 ${value("stalledWorkflowCount")}`}
            onClick={() => navigate("workflow-center")}
          />
        </div>
        <div className="dashboard-main-grid">
          <section className="panel dashboard-quick-panel">
            ...
            <div className="dashboard-facility-grid">
              {["zhujiang", "bioisland"].map((key) => (
                <FacilityCard key={key} facility={key} item={facilities.find((item) => item.facility === key)} />
              ))}
            </div>
          </section>
          <section className="panel dashboard-status-panel">
            ...
            <div className="status-chart">
              <div
                ...
                style={
                  {
                    "--active": percent(value("active"), total),
                    "--reserved": percent(value("reserved"), total),
                    "--empty": percent(value("empty"), total),
                  } as CSSProperties
                }
              >
        ...
        <section className="panel dashboard-room-panel">
          ...
          <div className="room-capacity-list">
            {data.roomSummaries.map((room) => (
              <RoomCapacity key={room.roomId} summary={room} room={roomsById.get(room.roomId)} />
            ))}
          </div>
        </section>
      </div>
...
function Meta({ label, value, tone = "neutral" }: { label: string; value: number; tone?: string }) {
  return (
    <div className={`workspace-meta-card ${tone}`}>
      <span>{label}</span>
      <strong>{value}</strong>
    </div>
  );
...
function Overview({
  ...
  return (
    <button className="dashboard-overview-card" type="button" onClick={onClick}>
      <span>{title}</span>
      <strong>{value}</strong>
      <small>{note}</small>
    </button>
...
function FacilityCard({ facility, item = {} }: { facility: string; item?: Record<string, unknown> }) {
  ...
  return (
    <article className="dashboard-facility-card">
      <div className="dashboard-facility-head">
        <div>
          <strong>{facility === "bioisland" ? "生物岛设施" : "珠江新城设施"}</strong>
          <span>{number("roomCount")} 个饲养间</span>
        </div>
        <span className="pill active">本月完成 {number("currentMonthWorkflowDoneCount")}</span>
      </div>
      <div className="dashboard-facility-metrics">
        <Summary label="在养笼数" value={number("activeCageCount")} />
        <Summary label="在养只数" value={number("activeAnimalCount")} />
        <Summary label="待进驻" value={number("openPlacementTaskCount")} />
      </div>
    </article>
  );
}
...
function RoomCapacity({
  ...
  summary: BootstrapResponse["roomSummaries"][number];
  ...
  return (
    <div className="room-capacity-row">
      <div className="room-capacity-head">
        <div>
          <strong>{String(room.name || summary.roomName || summary.roomId)}</strong>
          <span>
            {String(room.area || "未设置区域")} · {summary.slotCount} 笼
          </span>
        </div>
        <em>{occupiedPct}% 占用/预约</em>
      </div>
      <div className="room-capacity-meta">
        <span className="meta-pill active">在用 {summary.activeCount}</span>
        <span className="meta-pill reserved">已预约 {summary.reservedCount}</span>
        <span className="meta-pill empty">空 {summary.emptyCount}</span>
        <span className="meta-pill total">总笼位 {summary.slotCount}</span>
      </div>
    </div>
  );
}
    ...
    <section className="workspace-view dashboard-view">
      <section className="workspace-head">
        <div className="workspace-head-main">
          ...
          <p className="workspace-summary">正在加载运营数据...</p>
        </div>
      </section>
    </section>
  );
