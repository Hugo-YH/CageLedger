          ...
          <form className="panel large intake-entry-panel" onSubmit={submit}>
            ...
            <div className="intake-entry-layout">
              <div className="intake-message-field">
                <div className="intake-message-head">
                  <span>预约消息识别</span>
                  <button className="secondary compact-action" type="button" onClick={parseMessage}>
                    识别文本
                  </button>
                </div>
                <textarea
              ...
              <div className="intake-action-panel">
                ...
                <button
                  ...
                  type="button"
                  disabled={!draft.finalCardCount || save.isPending}
                  onClick={() => void printCurrentBatch()}
                >
                  打印当前笼卡
                </button>
              </div>
            ...
            <div className="intake-form-grid">
              ...
              <div className="intake-field-row two">
                ...
                <Field label="实验负责人/助手" value={draft.owner} onChange={(value) => update("owner", value)} />
              </div>
              <div className="intake-field-row four">
                <label>
                  物种
                  <select value={draft.species} onChange={(event) => update("species", event.target.value)}>
                ...
                <Field
                  ...
                  onChange={(value) => update("strainStandard", value)}
                />
                <Field
                  label="数量（只）"
                  required
                  ...
                  onChange={(value) => update("quantity", value ? Number(value) : null)}
                />
                <label>
                  房间
                  <select value={draft.roomName} onChange={(event) => update("roomName", event.target.value)}>
              ...
              <div className="intake-field-row three">
                <Field
                  ...
                  onChange={(value) => update("intakeDate", value)}
                />
                <Field
                  label="饲养周期（天）"
                  type="number"
                  value={draft.husbandryDays ?? ""}
                  onChange={(value) => update("husbandryDays", value ? Number(value) : null)}
                />
                <Field
                  label="结束日期"
                  type="date"
              ...
              <div className="intake-field-row three">
                <Field label="性别" value={draft.sex} onChange={(value) => update("sex", value)} />
                <Field
                  label="接收人员"
                  value={draft.receiverName}
                  onChange={(value) => update("receiverName", value)}
                />
                <Field label="兽医电话" value={draft.vetPhone} onChange={(value) => update("vetPhone", value)} />
              </div>
              <div className="intake-field-row two">
                <Field
                  label="打印张数"
                  ...
                  onChange={(value) => update("finalCardCount", Number(value) || 0)}
                />
                <label>
                  状态
                  <select
                    value={draft.status}
                    onChange={(event) => update("status", event.target.value as IntakeBatchStatus)}
                  >
              ...
              <div className="bulk-action-bar">
                ...
                <div>
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      if (openIntakeCardPrint(selectedItems)) void markPrinted(selectedItems);
                    }}
                  >
                    打印笼卡
                  </button>
                  <button className="secondary" type="button" onClick={() => void markPrinted(selectedItems)}>
                    标记已打印
                  </button>
                  <button className="secondary" type="button" onClick={() => void receive(selectedItems)}>
                    确认接收
                  </button>
                </div>
              </div>
                      ...
                      <tr key={item.id}>
                        ...
                        <td>
                          <div className="table-actions">
                            <button className="ghost info-button compact" type="button" onClick={() => edit(item)}>
                              编辑
                            </button>
                            <button
