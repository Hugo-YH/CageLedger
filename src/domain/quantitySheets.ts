...
const billingProfiles: Record<string, { item: string; species: string; unit: BillingUnit; internal: number; external: number }> = {
...
export function roomBillingProfile(room?: CageRoom) {
  ...
  return { ...profile, customerType, customerLabel: customerType === "external" ? "院外" : "院内", facilityLabel: room?.facility === "bioisland" ? "生物岛设施" : "珠江新城设施", price: customerType === "external" ? profile.external : profile.internal };
}

export function createQuantityRow(month: string, first = false): QuantitySheetRow {
  return { id: crypto.randomUUID(), date: first ? `${month}-01` : "", rawDateInput: first ? `${month}-01` : "", addedCount: null, addedType: "", transferInFromIacuc: "", removedCount: null, removedType: "", transferOutToIacuc: "", animalCount: null, cageCount: null, handler: "", balanceSource: "auto", notes: "" };
}

export function normalizeQuantityRow(row: Partial<QuantitySheetRow>, month: string): QuantitySheetRow {
  return { ...createQuantityRow(month), ...row, id: row.id || crypto.randomUUID(), date: String(row.date || ""), rawDateInput: String(row.rawDateInput ?? row.date ?? ""), addedCount: numberOrNull(row.addedCount), removedCount: numberOrNull(row.removedCount), animalCount: numberOrNull(row.animalCount), cageCount: numberOrNull(row.cageCount), addedType: String(row.addedType || ""), removedType: String(row.removedType || ""), transferInFromIacuc: String(row.transferInFromIacuc || "").trim().toUpperCase(), transferOutToIacuc: String(row.transferOutToIacuc || "").trim().toUpperCase(), balanceSource: row.balanceSource === "manual" ? "manual" : "auto" };
}
...
export function normalizeQuantitySheet(sheet: Partial<QuantitySheet>): QuantitySheet {
  ...
  return { id: sheet.id || crypto.randomUUID(), month, roomId: sheet.roomId || "", roomName: sheet.roomName || "", manager: sheet.manager || "", iacuc: String(sheet.iacuc || "").trim().toUpperCase(), project: sheet.project || "", pi: sheet.pi || "", owner: sheet.owner || "", contact: sheet.contact || "", funding: sheet.funding || "", preferredFreeCages: numberOrNull(sheet.preferredFreeCages), freeCagePriority: numberOrNull(sheet.freeCagePriority), billingUnit: sheet.billingUnit === "animal_day" ? "animal_day" : "cage_day", animalDetailEnabled: Boolean(sheet.animalDetailEnabled), initialAnimalCount: Number(sheet.initialAnimalCount || 0), initialCageCount: Number(sheet.initialCageCount || 0), pageCount: Math.max(Number(sheet.pageCount || 1), 1), rows: (sheet.rows || []).map((row) => normalizeQuantityRow(row, month)), updatedAt: sheet.updatedAt || "" };
}
...
export function validateQuantitySheet(sheet: QuantitySheet) {
  ...
  for (const row of sheet.rows) {
    ...
    if (row.removedType === "转出" && !row.transferOutToIacuc) issues.push(`${row.date || "未填日期"} 转出请填写伦理号`);
  }
  if (sheet.billingUnit === "animal_day" && !sheet.rows.some((row) => Number(row.animalCount || 0) > 0)) issues.push("按只/天计费房间必须填写动物结余总数");
  return issues;
}
