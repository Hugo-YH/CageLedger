...
let strainIndexMap: Record<string, string> | null = null;
...
async function strainIndex() {
  if (!strainIndexMap) {
    // Full MGI strain-name index regenerated by scripts/sync_mgi_strains.py.
    // Loaded lazily so the intake bundle never carries the 7.5MB table until
    // local recognition actually needs a full-index lookup; ?raw keeps
    // TypeScript from inferring the 117k-entry literal type.
    const module = await import("../../server_app/resources/intake/mgi-strain-index.json?raw");
    const index = JSON.parse(module.default) as { entries: Array<[string, string]> };
    strainIndexMap = {};
    for (const [key, name] of index.entries) {
      strainIndexMap[key] = name;
    }
  }
  return strainIndexMap;
}
...
export async function parseIntakeMessage(rawMessage: string, receiverName = "", roomNames: string[] = []) {
  ...
  return normalizeIntakeBatch(
    {
      ...
      strainRaw,
      strainStandard: (await standardizeStrain(strainRaw)) || strainRaw,
