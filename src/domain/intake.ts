...
function cleanField(value: string) {
  return value.replace(/^[:：，,\s]+/, "").split(/@|谢谢|请审核|请审批|老师/)[0].trim().replace(/[，,。；;]+$/, "").trim();
}
...
function normalizedDate(value: string, fallbackYear = new Date().getFullYear()) {
  ...
  const parts = full ? [Number(full[1]), Number(full[2]), Number(full[3])] : short ? [fallbackYear, Number(short[1]), Number(short[2])] : null;
...
export function normalizeIntakeBatch(item: Partial<IntakeBatch>, roomNames: string[] = []): IntakeBatch {
  const species = item.species || inferSpecies(`${item.strainStandard || ""} ${item.strainRaw || ""} ${item.rawMessage || ""}`);
  ...
  const suggestedCardCount = Math.max(numberOrNull(item.suggestedCardCount) || (quantity ? Math.ceil(quantity / perCage) : 0), 0);
  ...
  const status = validStatuses.has(item.status as IntakeBatchStatus) ? item.status as IntakeBatchStatus : "pending_print";
  ...
  return {
    id: String(item.id || crypto.randomUUID()), rawMessage: String(item.rawMessage || ""), purchaseOrderNo: String(item.purchaseOrderNo || "").trim(),
    batchNo: String(item.batchNo || "").trim().replace(/[（(]\s*([^()（）]+?)\s*[)）]/g, "（$1）"),
    iacuc: String(item.iacuc || "").trim().toUpperCase(), supplier: String(item.supplier || "").trim(), species,
    strainRaw: String(item.strainRaw || "").trim(), strainStandard: String(item.strainStandard || item.strainRaw || "").trim(), sex: String(item.sex || "").trim(), quantity,
    roomName, roomMatched: roomNames.length ? roomNames.includes(roomName) : Boolean(item.roomMatched), intakeDate,
    husbandryDays, endDate, project: String(item.project || "").trim(),
    pi: String(item.pi || "").trim(), owner: String(item.owner || "").trim(), receiverName: String(item.receiverName || "").trim(), vetPhone: String(item.vetPhone || "").trim(),
    notes: String(item.notes || "").trim(), status, suggestedAnimalsPerCage: perCage, suggestedCardCount, finalCardCount,
    confirmedCardCount, remainingCardCount: Math.max(numberOrNull(item.remainingCardCount) ?? finalCardCount - confirmedCardCount, 0),
    receipts: Array.isArray(item.receipts) ? item.receipts : [], cards: Array.isArray(item.cards) ? item.cards : [], updatedAt: String(item.updatedAt || ""),
  };
}
...
export function parseIntakeMessage(rawMessage: string, receiverName = "", roomNames: string[] = []) {
  ...
  const batchNo = batchMatch ? `（${batchMatch[1].toUpperCase()}）${batchMatch[2]}` : matchField(raw, ["饲养需求批次号", "批次号"]);
  const iacuc = batchMatch?.[1]?.toUpperCase() || "";
  const purchaseOrderNo = source.match(/锐[竞竟](?:采购)?(?:单号|订单编号|采购订单编号)?\s*(?:为)?\s*:?\s*([A-Z]{0,3}\d{8,})/i)?.[1] || "";
  const referenceYear = Number(batchMatch?.[2]?.slice(0, 4) || purchaseOrderNo.match(/20\d{2}/)?.[0] || new Date().getFullYear());
  const supplier = matchField(raw, ["供应商", "购买单位"]).split(/\s+(?:品系|数量|饲养房间)/)[0];
  const strainRaw = source.match(/品系\s*:?\s*([\s\S]*?)(?=\s*(?:数量|饲养房间|进驻日期|拟进驻日期|预计进驻日期)\s*:|[\n\r]|$)/i)?.[1]?.trim() || "";
  ...
  return normalizeIntakeBatch({ rawMessage: raw, purchaseOrderNo, batchNo, iacuc, supplier, strainRaw, strainStandard: strainRaw, species: inferSpecies(`${raw} ${strainRaw}`), sex: matchField(raw, ["性别"]), quantity, roomName, intakeDate: normalizedDate(dateText, referenceYear), husbandryDays, receiverName, status: "pending_print" }, roomNames);
}
