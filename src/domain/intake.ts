...
// Full MGI strain-name index regenerated by scripts/sync_mgi_strains.py; it
// covers every official MGI name, not only the curated common aliases above.
// Loaded via ?raw so TypeScript never infers the 117k-entry literal type.
import mgiStrainIndexRaw from "../../server_app/resources/intake/mgi-strain-index.json?raw";
...
function strainIndex() {
  if (!strainIndexMap) {
    strainIndexMap = {};
    const index = JSON.parse(mgiStrainIndexRaw) as { entries: Array<[string, string]> };
