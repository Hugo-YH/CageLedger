...
function renderQuantitySheetDetailModal() {
  ...
  return renderModalFrame({
    ...
    actions: mode === "edit" ? `<button id="saveQuantitySheetDetail" class="primary" type="submit" form="quantitySheetDetailForm">${iconSvg("save")}保存修改</button>` : "",
...
function readQuantitySheetForm(form, options = {}) {
  ...
  const rows = [...form.querySelectorAll("[data-quantity-field='date'][data-quantity-row]")]
    ...
    .map(({ rowCells, rowIndex, rawDateInput, date }) => {
      ...
      const autoCageCount = runningCageCount;
