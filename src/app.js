...
let pendingQuantitySheetSaveJob = null;
...
async function loadQuantitySheets(options = {}) {
  ...
  lazyDataState.quantitySheetsLoaded = true;
  lazyDataState.quantitySheetsLoading = false;
  const loadedIds = new Set(state.quantitySheets.map((sheet) => sheet.id));
  state.selectedQuantitySheetIds = state.selectedQuantitySheetIds.filter((id) => loadedIds.has(id));
  if (state.selectedQuantitySheetId && !loadedIds.has(state.selectedQuantitySheetId)) state.selectedQuantitySheetId = "";
  if (state.viewingQuantitySheetId && !loadedIds.has(state.viewingQuantitySheetId)) state.viewingQuantitySheetId = "";
...
function sortQuantitySheetsForList(items = []) {
  items.sort((left, right) => {
    const updatedDiff = String(right?.updatedAt || "").localeCompare(String(left?.updatedAt || ""));
    if (updatedDiff) return updatedDiff;
    const monthDiff = String(right?.month || "").localeCompare(String(left?.month || ""));
    if (monthDiff) return monthDiff;
    return String(left?.iacuc || "").localeCompare(String(right?.iacuc || ""));
  });
  return items;
}
...
function updateQuantitySheetListFromServer(sheet, previousSheet = null) {
  ...
  if (isVisible) {
    ...
    sortQuantitySheetsForList(state.quantitySheets);
    paginationState.quantitySheets.offset = 0;
    const limit = Number(paginationState.quantitySheets.limit || 0);
    if (limit > 0 && state.quantitySheets.length > limit) state.quantitySheets = state.quantitySheets.slice(0, limit);
  } else {
...
function renderConfirmDialog() {
  ...
  const dialogClass = state.confirmDialog.type ? ` confirm-dialog-${escapeAttr(state.confirmDialog.type)}` : "";
  return `
    <div class="confirm-dialog-backdrop" role="presentation">
      <section class="confirm-dialog${dialogClass}" role="dialog" aria-modal="true" aria-labelledby="confirmDialogTitle">
        <div>
          <h2 id="confirmDialogTitle">${escapeText(state.confirmDialog.title || "请确认")}</h2>
          ${renderConfirmDialogBody(state.confirmDialog)}
        </div>
...
function renderConfirmDialogBody(dialog) {
  if (dialog?.type === "save-quantity-sheet") {
    const billingParts = Array.isArray(dialog.payload?.billingParts) ? dialog.payload.billingParts : [];
    const statusItems = Array.isArray(dialog.payload?.statusItems) ? dialog.payload.statusItems : [];
    return `
      <div class="quantity-save-confirm">
        <section class="quantity-save-confirm-section">
          <span class="quantity-save-confirm-label">计费口径</span>
          <div class="quantity-save-confirm-chips">
            ${billingParts.map((part) => `<span>${escapeText(part)}</span>`).join("")}
          </div>
        </section>
        <section class="quantity-save-confirm-note">
          ${iconSvg("info")}
          <p>${escapeText(dialog.payload?.animalNotice || "")}</p>
        </section>
        <section class="quantity-save-confirm-stats" aria-label="数量统计表状态">
          ${statusItems
            .map((item) => `
              <div class="${item.tone ? `is-${escapeAttr(item.tone)}` : ""}">
  ...
  return `<p>${escapeText(dialog?.message || "")}</p>`;
}
...
function renderListMeta({ left = "", right = "" } = {}) {
...
function renderWorkspaceBody(content = "", className = "") {
  return `<div class="workspace-body ${escapeAttr(className)}">${content}</div>`;
}

function renderFieldCluster({ title = "", note = "", className = "", content = "" } = {}) {
  return `
    <section class="field-cluster ${escapeAttr(className)}">
      ${
        title || note
          ? `
            <div class="field-cluster-head">
              ${title ? `<strong>${escapeText(title)}</strong>` : ""}
              ${note ? `<span>${escapeText(note)}</span>` : ""}
            </div>
          `
          : ""
      }
      <div class="field-cluster-body">
        ${content}
      </div>
    </section>
  `;
...
function renderBulkActionBar({ selectedCount = 0, actions = "", label = "已选" } = {}) {
  if (!selectedCount) return "";
  return `
    <div class="bulk-action-bar" role="status" aria-live="polite">
      <strong>${escapeText(label)} ${selectedCount} 项</strong>
      <div>${actions}</div>
    </div>
  `;
}
...
function renderCageView() {
  ...
  return `
    <section class="workspace-view cage-workspace">
      ${renderWorkspaceHeader({
        kicker: "笼位运营工作台",
        title: "笼位管理",
        helpKey: "cages",
        summary: "按房间和笼架查看当前占用、预约、待进驻和饲养周期异常，选中笼位后集中编辑。",
        status: `${selectedRoom.name} · ${rackDisplayName(selectedRack, selectedRoom)}`,
        metrics: [
          { label: "当前笼架", value: `${visibleSlots.length} 笼` },
          { label: "待进驻", value: roomPlacementTasks.length, tone: roomPlacementTasks.length ? "warning" : "success" },
          { label: "已选笼位", value: state.batchMode ? state.selectedSlotIds.length : selectedSlot ? 1 : 0, tone: state.batchMode && state.selectedSlotIds.length ? "todo" : "neutral" },
        ],
      })}
      ${renderWorkspaceBody(`
    ...
    <section class="workspace-view intake-workspace">
      ...
      ${renderWorkspaceBody(`
        <section class="billing-layout quantity-billing-layout intake-layout">
          <div class="panel large intake-entry-panel">
...
function renderIntakeBatchEditorModal() {
  ...
  if (!draft) return "";
  return renderModalFrame({
    backdropId: "closeIntakeBatchEditor",
    closeButtonId: "closeIntakeBatchEditorButton",
    className: "intake-batch-editor-modal",
    title: "编辑待接收批次",
...
function renderQuantitySheetBillingView() {
  ...
  const animalDetails = quantitySheetAnimalDetailState(draft, quantityProfile);
  const saveHint = quantitySheetSaveButtonHint(draft);
        ...
        <form id="quantitySheetForm">
          ...
          <div class="quantity-sheet-fields">
            ${renderFieldCluster({
              title: "基础信息",
              note: "月份和房间决定计费口径。",
              className: "quantity-field-cluster",
              content: `
            ...
            ${renderFieldCluster({
              title: "项目与伦理",
              note: "选择 IACUC 后自动带出项目信息，也支持手动修正。",
              className: "quantity-field-cluster",
              content: `
...
function renderQuantitySheetDetailModal() {
  ...
  const saveHint = quantitySheetSaveButtonHint(sheet);
  return renderModalFrame({
...
function renderSavedQuantitySheetEditor(sheet) {
  ...
  const quantityProfile = billingProfileForRoom(quantityRoom || {});
  const animalDetails = quantitySheetAnimalDetailState(sheet, quantityProfile);
  return `
    <div class="quantity-sheet-detail-workbench">
      <form id="quantitySheetDetailForm" class="quantity-sheet-detail-form" data-sheet-id="${escapeAttr(sheet.id)}">
        <div class="quantity-detail-action-strip">
          ${renderQuantityAnimalDetailToggle(animalDetails)}
        </div>
        <div class="quantity-sheet-fields">
          ${renderFieldCluster({
    ...
    <section class="workflow-center-panel">
      <div class="panel">
        ${renderPanelHead({
          ...
          actions: `
          ${renderCommandBar({
            className: "workflow-filter-toolbar reimbursement-toolbar",
            filters: `
              <div class="filter-row" role="group" aria-label="报销台账状态筛选">
        ...
        ${renderListMeta({
          left: workflowLoading ? "正在加载报销台账" : `当前命中 ${items.length} / 已加载 ${state.reimbursementRecords.length} 条`,
          right: remotePersistence ? paginationSummary(workflowPage) : "",
        })}
        <div class="table-wrap">
          <table class="workflow-table reimbursement-table dense-table">
            <thead>
              <tr>
                <th>结算月份</th>
                <th>项目负责人</th>
...
function renderBillingWorkflowDetailModal() {
  ...
  return renderModalFrame({
    backdropId: "closeReimbursementDetail",
    closeButtonId: "closeReimbursementDetailButton",
    className: "workflow-detail-modal",
    title: record.pi || "报销台账",
    subtitle: `${record.month || "-"} · 当前月应缴 ¥${MONEY_FORMAT.format(Number(record.payableAmount || 0))} · 累计未缴 ¥${MONEY_FORMAT.format(Number(record.accumulatedUnpaid || 0))}`,
    helpKey: "reimbursementLedger",
    actions: `
...
function quantitySheetHasAnimalDetails(sheet) {
  return (sheet?.rows || []).some((row) =>
    numericOrZero(row?.addedCount) > 0
    || numericOrZero(row?.removedCount) > 0
    || row?.addedType
    || row?.removedType
    || row?.transferInFromIacuc
    || row?.transferOutToIacuc
    || row?.animalCount !== null && row?.animalCount !== undefined,
  );
}

function quantitySheetAnimalDetailState(sheet, profile = null) {
  const draft = normalizeQuantitySheetDraft(sheet || {});
  const roomId = quantitySheetRoomId(draft);
  const resolvedProfile = profile || billingProfileForRoom(state.rooms.find((room) => room.id === roomId) || {});
  const billingUnit = profile || roomId ? resolvedProfile.unit : draft.billingUnit;
  const required = billingUnit === "animal_day";
  const explicit = sheet?.animalDetailEnabled;
  const enabled = required || explicit === true || (explicit !== false && quantitySheetHasAnimalDetails(draft));
  return {
    enabled,
    required,
    locked: required,
    billingUnit,
    note: required ? "按只/天计费，必须维护动物数量；结余笼数默认按 1 只 1 笼同步。" : "按笼/天计费，动物数量仅作辅助台账。",
  };
}

function renderQuantityAnimalDetailToggle(stateInfo) {
  const stateClass = stateInfo.enabled ? "enabled" : "disabled";
  const noticeText = quantityAnimalDetailNoticeText(stateInfo);
  return `
    <label class="quantity-animal-toggle ${stateClass} ${stateInfo.locked ? "locked" : ""}" title="${escapeAttr(noticeText)}">
      <input type="checkbox" data-quantity-animal-toggle ${stateInfo.enabled ? "checked" : ""} ${stateInfo.locked ? "disabled" : ""} />
      <span class="quantity-animal-toggle-track" aria-hidden="true"><span></span></span>
      <span class="quantity-animal-toggle-label">动物数量</span>
      ${stateInfo.locked ? `<small>必填</small>` : ""}
    </label>
  `;
}

function quantityAnimalDetailNoticeText(stateInfo) {
  return stateInfo.required
    ? "当前房间按只/天计费，动物数量明细已锁定打开；结余笼数默认等于结余总数，可按实际笼数修正。"
    : stateInfo.enabled
      ? "动物数量明细已打开：可录入增加、减少和结余总数；本房间仍按结余笼数计费。"
      : "动物数量明细已关闭：当前只录入日期和结余笼数，系统按笼/天计费。";
}

function showQuantityAnimalDetailNotice(sheet) {
  const stateInfo = quantitySheetAnimalDetailState(sheet);
  const title = stateInfo.required
    ? "动物数量明细必填"
    : stateInfo.enabled
      ? "动物数量明细已打开"
      : "动物数量明细已关闭";
  showFlashNotice(title, quantityAnimalDetailNoticeText(stateInfo), stateInfo.enabled || stateInfo.required ? "success" : "warning");
}
...
function quantitySheetStatusSummaryText(draft) {
  const { filledRows, issueRows, transferRows } = quantityEntryStatusFromDraft(draft);
  return [
    `数量统计表页数：${quantitySheetPageCount(draft)} 页`,
    `变更 ${filledRows} 行`,
    `转移 ${transferRows} 行`,
    issueRows ? `待检查 ${issueRows} 行` : "无待检查行",
  ].join("\n");
}

function quantitySheetStatusItems(draft) {
  const { filledRows, issueRows, transferRows } = quantityEntryStatusFromDraft(draft);
  return [
    { label: "页数", value: `${quantitySheetPageCount(draft)} 页` },
    { label: "变更", value: `${filledRows} 行` },
    { label: "转移", value: `${transferRows} 行` },
    { label: "检查", value: issueRows ? `${issueRows} 行待检查` : "无待检查行", tone: issueRows ? "warning" : "ok" },
  ];
}

function quantitySheetSaveButtonHint(draft) {
  return quantitySheetStatusSummaryText(draft);
}

function quantitySheetBillingProfileText(sheet) {
  return quantitySheetBillingProfileParts(sheet).join(" / ");
}

function quantitySheetBillingProfileParts(sheet) {
  const normalized = normalizeQuantitySheetDraft(sheet || {});
  const profile = quantitySheetBillingProfile(normalized);
  const room = state.rooms.find((item) => item.id === quantitySheetRoomId(normalized));
  return [
    facilityLabel(profile.facility),
    billingItemLabel(profile.billingItem),
    customerTypeLabel(profile.customerType),
    `单价 ${billingPriceLabel(profile)}`,
    room?.name ? `房间 ${room.name}` : "",
  ].filter(Boolean);
}

function quantitySheetSaveConfirmMessage(draft) {
  const animalNotice = quantityAnimalDetailNoticeText(quantitySheetAnimalDetailState(draft));
  return [
    `计费口径：${quantitySheetBillingProfileText(draft)}`,
    animalNotice,
    quantitySheetStatusSummaryText(draft),
  ].join("\n");
}

function openQuantitySheetSaveConfirm({ mode, draft }) {
  pendingQuantitySheetSaveJob = { mode, draft };
  const animalNotice = quantityAnimalDetailNoticeText(quantitySheetAnimalDetailState(draft));
  openConfirmDialog({
    type: "save-quantity-sheet",
    title: "确认保存数量统计表",
    message: quantitySheetSaveConfirmMessage(draft),
    confirmLabel: "确认保存",
    confirmClass: "primary",
    payload: {
      billingParts: quantitySheetBillingProfileParts(draft),
      animalNotice,
      statusItems: quantitySheetStatusItems(draft),
    },
  });
}
...
function syncQuantityEntryStatus(form = document.querySelector("#quantitySheetForm")) {
  ...
  const saveButton = form?.querySelector?.("#saveQuantitySheet, #saveQuantitySheetDetail");
  if (saveButton) {
    const pageCount = form.id === "quantitySheetDetailForm"
      ? quantitySheetPageCount(captureQuantitySheetDetailDraft(form))
      : quantitySheetPageCount(state.quantitySheetDraft || {});
    saveButton.title = [
      `数量统计表页数：${pageCount} 页`,
      `变更 ${stats.filledRows} 行`,
      `转移 ${stats.transferRows} 行`,
      stats.issueRows ? `待检查 ${stats.issueRows} 行` : "无待检查行",
    ].join("\n");
  }
  if (!status) return;
...
function renderQuantitySheetCalendarPages(draft) {
  ...
  const animalDetails = quantitySheetAnimalDetailState(draft);
  ...
  return Array.from({ length: pages }, (_, pageIndex) => {
    ...
    return `
      <div class="quantity-template-page">
        ...
        <div class="table-wrap">
          <table class="quantity-entry-table quantity-template-table ${animalDetails.enabled ? "animal-details-enabled" : "animal-details-hidden"}">
            <thead>
              <tr>
                <th><span class="quantity-table-head-label">日期${renderHelpHint("quantityDateFormat")}</span></th>
                <th class="animal-detail-col">新增<br />（购/转/分）</th>
                <th class="animal-detail-col">减少<br />（取/死/转）</th>
                <th class="animal-detail-col">结余<br />总数</th>
                <th>结余<br />笼数</th>
                <th><span class="quantity-table-head-label">日期${renderHelpHint("quantityDateFormat")}</span></th>
                <th class="animal-detail-col">新增<br />（购/转/分）</th>
                <th class="animal-detail-col">减少<br />（取/死/转）</th>
...
function renderQuantitySheetCalendarCell(entry) {
  if (!entry) return `<td></td><td class="animal-detail-col"></td><td class="animal-detail-col"></td><td class="animal-detail-col"></td><td></td>`;
  const row = entry.row || {};
  ...
  return `
    ...
    <td class="quantity-change-cell animal-detail-col" data-quantity-row="${escapeAttr(entry.index)}" data-quantity-field="added">${renderQuantityChangeStack("added", entry)}</td>
    <td class="quantity-change-cell animal-detail-col" data-quantity-row="${escapeAttr(entry.index)}" data-quantity-field="removed">${renderQuantityChangeStack("removed", entry)}</td>
    <td class="animal-detail-col" data-quantity-row="${escapeAttr(entry.index)}" data-quantity-field="animal">
...
function buildQuantitySheetCalendarRows(sheet) {
  ...
  const animalDetails = quantitySheetAnimalDetailState(draft, profile);
  ...
  return groupedSourceRows.map((item) => {
    ...
    const autoCageCount = animalDetails.required ? autoAnimalCount : cageCount;
...
function renderRoomManagementView() {
  ...
  return `
    <section class="workspace-view settings-workspace">
      ${renderWorkspaceHeader({
        kicker: "基础设施工作台",
        title: "房间管理",
        helpKey: "rooms",
        summary: "维护饲养间、笼架和笼位规模，供笼位录入、数量统计表和饲养费核算共同使用。",
        status: canManageRooms ? "系统管理员可维护" : "当前账号只读",
        metrics: [
          { label: "饲养间", value: visibleRooms().length },
          { label: "笼架", value: state.racks.length },
          { label: "笼位", value: state.slots.length },
        ],
      })}
      ${renderWorkspaceBody(`
      <section class="content-grid">
  ...
  return `
    <section class="workspace-view settings-workspace">
      ${renderWorkspaceHeader({
        ...
        title: "账号管理",
        helpKey: "users",
        summary: "维护系统管理员和房间管理员，房间授权直接影响笼位和饲养费录入范围。",
        status: `${users.length} 个账号`,
        metrics: [
          { label: "系统管理员", value: users.filter((user) => user.role === "admin").length },
          { label: "房间管理员", value: users.filter((user) => user.role === "room_admin").length },
          { label: "饲养间", value: state.rooms.length },
        ],
      })}
      ${renderWorkspaceBody(`
      <section class="content-grid">
  ...
  return `
    <section class="workspace-view settings-workspace">
      ${renderWorkspaceHeader({
        ...
        title: "数据管理",
        helpKey: "data",
        summary: "维护 IACUC 索引、项目负责人身份和历史报销台账导入，保障录入与结算自动匹配。",
        status: `${iacucIndexMeta?.count ?? IACUC_INDEX.length} 条 IACUC`,
        metrics: [
          { label: "IACUC 索引", value: iacucIndexMeta?.count ?? IACUC_INDEX.length },
          { label: "负责人身份", value: principalIdentityRows().length },
          { label: "导入入口", value: "CSV / Excel", tone: "todo" },
        ],
      })}
      ${renderWorkspaceBody(`
      <section class="content-grid">
  ...
  return `
    <section class="workspace-view settings-workspace">
      ${renderWorkspaceHeader({
        ...
        title: "操作日志",
        helpKey: "logs",
        summary: "查看系统写入操作、操作者和对象变更，辅助追踪数据维护来源。",
        status: loading ? "正在加载" : `${logs.length} 条已加载`,
        metrics: [
          { label: "当前页", value: logs.length },
          { label: "总记录", value: remotePersistence ? page.total || logs.length : logs.length },
          { label: "模式", value: remotePersistence ? "共享" : "本地", tone: "todo" },
        ],
      })}
      ${renderWorkspaceBody(`
...
function paginationSummary(pageState) {
  const total = Math.max(Number(pageState.total) || 0, 0);
  const page = currentPage(pageState);
  const pages = pageCount(total, pageState.limit);
...
function bindEvents() {
  ...
  document.querySelector("#quantitySheetDetailForm")?.addEventListener("change", (event) => {
    const name = event.target?.name;
    if (event.target?.matches?.("[data-quantity-animal-toggle]")) {
      const form = event.currentTarget;
      form.elements.animalDetailEnabled.value = event.target.checked ? "true" : "false";
      const draft = captureQuantitySheetDetailDraft(form);
      showQuantityAnimalDetailNotice(draft);
      scheduleRender("quantity_sheet.detail.animal_detail");
      return;
    }
  ...
  document.querySelector("#quantitySheetForm")?.addEventListener("focusin", handleQuantityEntryFocusIn);
  ...
  document.querySelector("#quantitySheetForm")?.addEventListener("change", (event) => {
    ...
    if (event.target?.matches?.("[data-quantity-animal-toggle]")) {
      ...
      showQuantityAnimalDetailNotice(state.quantitySheetDraft);
      scheduleRender("quantity_sheet.animal_detail");
      return;
    }
    if (name === "quantityDatePicker") {
      applyQuantityDatePickerValue(event.target);
...
function handleQuantityEntryFocusIn(event) {
  const target = event.target;
  if (!(target instanceof HTMLInputElement || target instanceof HTMLSelectElement)) return;
  setActiveQuantityEntryCell(target);
}

function handleQuantityEntryFocusOut(event) {
  const wrap = event.target?.closest?.(".quantity-entry-wrap");
  if (!wrap) return;
  window.setTimeout(() => {
    if (!wrap.contains(document.activeElement)) clearActiveQuantityEntryCell(wrap);
  }, 0);
}

function setActiveQuantityEntryCell(target) {
  const cell = target.closest?.("td[data-quantity-row]");
  const wrap = target.closest?.(".quantity-entry-wrap");
  if (!cell || !wrap) return;
  clearActiveQuantityEntryCell(wrap);
  const row = cell.closest("tr");
  const isLeftSide = cell.cellIndex < 5;
  row?.classList.add(isLeftSide ? "quantity-row-active-left" : "quantity-row-active-right");
  cell.classList.add("quantity-cell-active");
}

function clearActiveQuantityEntryCell(scope = document) {
  scope.querySelectorAll?.(".quantity-row-active-left, .quantity-row-active-right").forEach((row) => {
    row.classList.remove("quantity-row-active-left", "quantity-row-active-right");
  });
...
function focusPendingQuantityField() {
  ...
  window.setTimeout(() => {
    ...
    if (input instanceof HTMLInputElement) {
      input.focus();
      input.select();
      setActiveQuantityEntryCell(input);
      pendingQuantityFocusTarget = null;
    }
  }, 80);
}

function syncQuantityTransferLookupEditor(select) {
  if (!(select instanceof HTMLSelectElement)) return null;
  const editor = select.closest(".quantity-change-editor");
  const cell = select.closest("td");
  const form = select.closest("form");
  if (!editor || !cell || !form) return null;
  const isAdded = select.name === "addedType";
  const transferType = isAdded ? "转入" : "转出";
  const inputName = isAdded ? "transferInFromIacuc" : "transferOutToIacuc";
  const placeholder = isAdded ? "来源伦理号" : "目标伦理号";
...
function focusFirstQuantitySheetProblem({ invalidDateInputs = [], form = null } = {}) {
  ...
  if (!target) return;
  const cell = target.closest("td");
  const attentionTarget = cell || target;
  if (quantityIssueAttentionTimer) clearTimeout(quantityIssueAttentionTimer);
  document.querySelectorAll(".quantity-cell-attention").forEach((item) => item.classList.remove("quantity-cell-attention"));
  attentionTarget.classList.add("quantity-cell-attention");
  ...
  quantityIssueAttentionTimer = window.setTimeout(() => {
    attentionTarget.classList.remove("quantity-cell-attention");
    quantityIssueAttentionTimer = null;
  }, 2600);
}

function queueQuantitySheetProblemFocus(options = {}) {
  if (pendingQuantityProblemFocusTimer) clearTimeout(pendingQuantityProblemFocusTimer);
  pendingQuantityProblemFocusTimer = window.setTimeout(() => {
    pendingQuantityProblemFocusTimer = null;
    focusFirstQuantitySheetProblem(options);
  }, 160);
}
...
function bindIacucLookupInputs(scopeSelector, autofillHandler) {
  ...
  document.querySelectorAll(`${scopeSelector} [data-iacuc-lookup]`).forEach((input) => {
    if (input.dataset.iacucLookupBound === "true") return;
    input.dataset.iacucLookupBound = "true";
  ...
  if (scope && scope.dataset.iacucLookupPickBound !== "true") {
    scope.dataset.iacucLookupPickBound = "true";
...
async function handleQuantitySheetSubmit(event) {
  ...
  try {
    const form = event.currentTarget;
    const draft = readQuantitySheetForm(form, { preserveInvalidDates: true });
    state.quantitySheetDraft = draft;
    openQuantitySheetSaveConfirm({ mode: "main", draft });
  } catch (error) {
    reportSaveError(error);
  }
}
...
async function handleQuantitySheetDetailSubmit(event) {
  ...
  try {
    ...
    openQuantitySheetSaveConfirm({ mode: "detail", draft });
  } catch (error) {
    reportSaveError(error);
  }
}

async function executePendingQuantitySheetSave() {
  const job = pendingQuantitySheetSaveJob;
  pendingQuantitySheetSaveJob = null;
  if (!job?.draft) return;
  if (job.mode === "detail") {
    const savedSheet = await saveQuantitySheetObject(job.draft, { updateDraft: false });
  ...
  const savedSheet = await saveQuantitySheetObject(job.draft, { startedAt: performance.now(), updateDraft: true });
...
function toggleQuantitySheetSelection(sheetId, checked) {
  ...
  if (checked) selected.add(sheetId);
  else selected.delete(sheetId);
  state.selectedQuantitySheetIds = [...selected];
  scheduleRender("quantity_sheet.selection");
}
...
function toggleVisibleQuantitySheetSelection(checked) {
  ...
  state.selectedQuantitySheetIds = [...selected];
  scheduleRender("quantity_sheet.selection.visible");
}
...
function deleteCurrentQuantitySheet() {
  const selectedSheets = selectedQuantitySheetsForAction();
  if (!selectedSheets.length) return;
  const label = selectedSheets.length === 1
    ? [selectedSheets[0].iacuc, selectedSheets[0].month].filter(Boolean).join(" ")
    : `${selectedSheets.length} 张数量统计表`;
  openConfirmDialog({
    type: "delete-quantity-sheets",
    id: selectedSheets[0].id,
    title: selectedSheets.length === 1 ? "删除数量统计表" : "批量删除数量统计表",
...
async function deleteQuantitySheetConfirmed(sheetId) {
  ...
  if (state.viewingQuantitySheetId === sheetId) state.viewingQuantitySheetId = "";
  invalidateStateIndexCache();
  scheduleRender("quantity_sheet.delete.remote");
}

async function deleteQuantitySheetsConfirmed(sheetIds = []) {
  for (const sheetId of sheetIds) {
    await deleteQuantitySheetConfirmed(sheetId);
  }
}

function selectedQuantitySheetsForAction() {
  const selectedIds = new Set(state.selectedQuantitySheetIds || []);
  return state.quantitySheets.filter((sheet) => selectedIds.has(sheet.id));
}

async function selectedQuantitySheetForSingleAction() {
  const selected = selectedQuantitySheetsForAction();
  if (selected.length !== 1) throw new Error("请只勾选一张数量统计表后再操作");
  return (await ensureQuantitySheetDetail(selected[0])) || selected[0];
}
...
async function saveQuantitySheetObject(sheetDraft, { startedAt = performance.now(), updateDraft = false } = {}) {
  ...
  if (sheetDraft.invalidDateInputs?.length) {
    queueQuantitySheetProblemFocus({ invalidDateInputs: sheetDraft.invalidDateInputs });
  ...
  if (inputIssues.length) {
    ...
    queueQuantitySheetProblemFocus();
...
function readQuantitySheetForm(form, options = {}) {
  ...
  const rows = [...form.querySelectorAll("[data-quantity-field='date'][data-quantity-row]")]
    .map((cell) => {
      ...
      const rawDateInput = String(rowCells.date?.querySelector("[name='rowDate']")?.value || "").trim();
      const date = rowIndex === 0 ? `${month}-01` : normalizeQuantitySheetEntryDate(rawDateInput, month);
      return { cell, rowCells, rowIndex, rawDateInput, date };
    })
    .sort((a, b) => {
      if (a.date && b.date) return String(a.date || "").localeCompare(String(b.date || "")) || a.rowIndex - b.rowIndex;
      if (a.date) return -1;
      if (b.date) return 1;
    ...
    .flatMap(({ rowCells, rowIndex, rawDateInput, date }) => {
      ...
      const addedEntries = animalDetailEnabled ? readQuantityChangeEditors(rowCells.added, "added") : [];
      const removedEntries = animalDetailEnabled ? readQuantityChangeEditors(rowCells.removed, "removed") : [];
      ...
      let manualAnimalCount = animalDetailEnabled ? manualQuantityBalanceValue(animalInput) : null;
      ...
      const primaryManual = manualAnimalCount;
      const primaryPrevious = runningAnimalCount;
      ...
      const autoCageCount = profile.unit === "animal_day" && animalDetailEnabled ? autoAnimalCount : runningCageCount;
      ...
      if (profile.unit === "animal_day" && animalDetailEnabled && manualCageCount === null) manualCageCount = autoCageCount;
      if (rowIndex === 0) {
        manualAnimalCount = animalDetailEnabled ? numericOrZero(animalInput?.value) : null;
        manualCageCount = numericOrZero(cageInput?.value);
        if (profile.unit === "animal_day" && animalDetailEnabled && manualCageCount === 0 && numericOrZero(animalInput?.value) > 0) manualCageCount = numericOrZero(animalInput?.value);
      }
...
function quantitySheetBillingProfile(sheet) {
  const normalized = normalizeQuantitySheetDraft(sheet || {});
  const room = state.rooms.find((item) => item.id === quantitySheetRoomId(normalized));
  if (room) return billingProfileForRoom(room);
  return {
    ...billingProfileForRoom({}),
    unit: normalized.billingUnit === "animal_day" ? "animal_day" : "cage_day",
  };
}

function quantitySheetHasRequiredAnimalBalance(sheet) {
  return (sheet?.rows || []).some((row) => row?.animalCount !== null && row?.animalCount !== undefined && numericOrZero(row.animalCount) > 0);
}

function assertQuantitySheetAnimalRequirements(sheet) {
  const normalized = normalizeQuantitySheetDraft(sheet || {});
  const profile = quantitySheetBillingProfile(normalized);
  if (profile.unit !== "animal_day") return;
  if (quantitySheetHasRequiredAnimalBalance(normalized)) return;
  throw new Error("该房间按只/天计费，请打开动物数量并补充结余总数。");
}

function quantitySheetOptionalAnimalNotice(sheet) {
  const normalized = normalizeQuantitySheetDraft(sheet || {});
  const profile = quantitySheetBillingProfile(normalized);
  if (profile.unit !== "cage_day") return "";
  if (quantitySheetHasAnimalDetails(normalized)) return "";
  return "数量统计表已保存；当前未填写动物数量，系统将仅按结余笼数计算饲养费。";
}
...
function normalizeQuantitySheetDraft(sheet) {
  ...
  return {
    ...
    invalidDateInputs: Array.isArray(sheet?.invalidDateInputs) ? sheet.invalidDateInputs.filter(Boolean) : [],
...
async function persistBillingWorkflowFromCurrent() {
  ...
  if (state.billingSource === "quantity_sheet") {
    const sheet = await selectedQuantitySheetForSingleAction();
...
function quantityChangeCell(rows, kind) {
  ...
  return rows
    ...
    .map((row) => {
      ...
      const type = quantityChangeFullType(row[typeKey], kind);
      if (type === "转入") {
        const sourceIacuc = normalizeIacucNumber(row.transferInFromIacuc || "");
        return sourceIacuc ? `${count}（转入 ${sourceIacuc}）` : `${count}（转入）`;
      }
      if (type === "转出") {
        const targetIacuc = normalizeIacucNumber(row.transferOutToIacuc || "");
        return targetIacuc ? `${count}（转出 ${targetIacuc}）` : `${count}（转出）`;
      }
      const shortType = quantityChangeShortLabel(type);
      return shortType ? `${count}（${shortType}）` : count;
    })
    .join("；");
}
  ...
  for (const iacuc of iacucs) {
    ...
    timelines.set(
      ...
      (statement.rows || []).map((row) => {
        ...
        const balanceValue = current.animalCount;
...
function quantityStatisticPagesHtml(statement, forms) {
...
function closeConfirmDialog() {
  ...
  if (state.confirmDialog?.type === "save-quantity-sheet") {
    pendingQuantitySheetSaveJob = null;
  }
...
async function handleConfirmDialogAction() {
  ...
  try {
    ...
    if (dialog.type === "save-quantity-sheet") {
      await executePendingQuantitySheetSave();
      return;
    }
    ...
    if (dialog.type === "delete-quantity-sheets") {
      await deleteQuantitySheetsConfirmed(dialog.payload?.ids || []);
      pushLog(`删除数量统计表：${dialog.payload?.label || dialog.id}`);
      showFlashNotice("删除成功", `数量统计表已删除：${dialog.payload?.label || dialog.id}`);
      return;
    }
...
function iconSvg(name) {
  const icons = {
    ...
    info: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M11 10h2v8h-2zm0-4h2v2h-2z"/><path d="M12 2a10 10 0 1 1 0 20 10 10 0 0 1 0-20zm0 2a8 8 0 1 0 0 16 8 8 0 0 0 0-16z"/></svg>`,
